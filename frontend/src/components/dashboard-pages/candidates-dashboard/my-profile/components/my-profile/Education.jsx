...
const Education = ({
  // These booleans control whether corresponding Grade 12 fields render
  isEasyMode,
  grade12syllabus,
  grade12school,
  grade12percentage,
  ...
  // Options for core subjects, degrees, etc.
  const [coreSubjectsOptions, setCoreSubjectsOptions] = useState([]);
  const [degrees, setDegrees] = useState([]);
  const [masterDegrees, setMasterDegrees] = useState([]);

  // Holds all education sections beyond Grade 10
  const [additionalEducation, setAdditionalEducation] = useState([]);

  // List of education section types
  const educationTypes = [
    { value: "grade12", label: "Grade 12", allowMultiple: false },
    { value: "degree", label: "Degree", allowMultiple: true },
    { value: "masterDegree", label: "Master Degree", allowMultiple: true },
  ...
  // ------------- GET: Fetch education details from the API -------------
  useEffect(() => {
    if (!user?.uid) return;
    ...
    const fetchEducationDetails = async () => {
      try {
        ...
        if (response.status === 200 && response.data) {
          // If API returns an array, pick the first record
          const record = Array.isArray(response.data) ? response.data[0] : response.data;

          // ---------- Parse grade10 data ----------
          let grade10 = record.grade10 || {
            syllabus: "",
            schoolName: "",
            yearOfPassing: "",
            percentage: "",
          ...
          if (typeof grade10 === "string") {
            try {
              grade10 = JSON.parse(grade10);
            } catch (e) {
              console.error("Error parsing grade10:", e);
            }
          }
          setGrade10Data(grade10);

          // ---------- Parse additionalEducation ----------
          let additionalEdu = record.additionalEducation || [];
          if (typeof additionalEdu === "string") {
            try {
              additionalEdu = JSON.parse(additionalEdu);
            } catch (e) {
              console.error("Error parsing additionalEducation:", e);
            }
          }

          // ---------- (NEW) Check for top-level "grade12" ----------
          // If your API returns grade12 outside of additionalEducation, add it in
          if (record.grade12) {
            let grade12Obj = record.grade12;
            if (typeof grade12Obj === "string") {
              try {
                grade12Obj = JSON.parse(grade12Obj);
              } catch (e) {
                console.error("Error parsing grade12:", e);
              }
            }
            // Only add if an item with type "grade12" isn’t already in additionalEdu
            const exists = additionalEdu.some((item) => item.type === "grade12");
            if (!exists) {
              additionalEdu.push({
                type: "grade12",
                data: grade12Obj
          ...
          setAdditionalEducation(additionalEdu);
          console.log("Final additionalEducation:", additionalEdu);
        }
      } catch (error) {
  ...
  // Provide default data for each education type
  const getInitialDataForType = (type) => {
    switch (type) {
      case "grade12":
        return {
  ...
  useEffect(() => {
    const fetchDegrees = async () => {
      try {
        ...
        setDegrees(
          transformedData.filter((item) => item.category === "Degrees") || []
        );
        setMasterDegrees(
          transformedData.filter((item) => item.category === "MasterDegree") || []
        );
  ...
  // Renders education fields based on type
  const renderEducationFields = (type, data, index) => {
    switch (type) {
      case "grade12":
        return (
          <div className="row">
            ...
            {grade12school && (
              <div className="form-group col-lg-6 col-md-12">
                <input
                  ...
                  value={data.schoolName}
                  onChange={(e) =>
                    handleEducationDataChange(index, "schoolName", e.target.value)
                  }
                  placeholder="School Name"
                  pattern="[a-zA-Z0-9 ]*"
                  maxLength={20}
            ...
            <div className="form-group col-lg-6 col-md-12">
              ...
              >
                ...
                {(() => {
                  ...
                  const minYear = currentYear - 14;
                  const maxYear =
                    data.courseStatus === "Pursuing" ? currentYear + 2 : currentYear;
                  for (let year = maxYear; year >= minYear; year--) {
                    years.push(
                      <option key={year} value={year}>
                        {year}
            ...
            <div className="form-group col-lg-6 col-md-12">
              <Select
                isMulti
                value={data.coreSubjects.map((subject) => ({
                  value: subject,
                  label: subject
                }))}
                onChange={(selected) => {
            ...
            {data.coreSubjects.includes("Others") && (
              <div className="form-group col-lg-6 col-md-12">
                <input
                  ...
                  value={data.otherSubjects}
                  onChange={(e) =>
                    handleEducationDataChange(index, "otherSubjects", e.target.value)
                  }
                  placeholder="Specify other subjects"
                  required
                />
            ...
            {grade12percentage && (
              <div className="form-group col-lg-6 col-md-12">
                <input
                  ...
                  value={data.percentage}
                  onChange={(e) =>
                    handleEducationDataChange(index, "percentage", e.target.value)
                  }
                  placeholder="Grade / Percentage"
                  pattern="[a-zA-Z0-9+%]*"
                  maxLength={5}
            ...
            {grade12mode && (
              <div className="form-group col-lg-6 col-md-12">
                <select
                  ...
                  value={data.mode || ""}
                  onChange={(e) =>
                    handleEducationDataChange(index, "mode", e.target.value)
                  }
                  required
                >
                  <option value="" disabled>
              ...
              {degreeCollege && (
                <div className="form-group col-lg-6 col-md-12">
                  <input
                    ...
                    value={data.collegeName}
                    onChange={(e) =>
                      handleEducationDataChange(index, "collegeName", e.target.value)
                    }
                    placeholder="College Name"
                    pattern="[a-zA-Z0-9 ]*"
                    maxLength={20}
              ...
              {degreePlace && (
                <div className="form-group col-lg-6 col-md-12">
                  <input
                    ...
                    value={data.placeOfStudy}
                    onChange={(e) =>
                      handleEducationDataChange(index, "placeOfStudy", e.target.value)
                    }
                    placeholder="Place of Study"
                    maxLength={20}
                    pattern="[a-zA-Z0-9 ]*"
              ...
              {degreeUniversity && (
                <div className="form-group col-lg-6 col-md-12">
                  <input
                    ...
                    value={data.universityName}
                    onChange={(e) =>
                      handleEducationDataChange(index, "universityName", e.target.value)
                    }
                    placeholder="University Name"
                    maxLength={20}
                  />
              ...
              <div className="form-group col-lg-6 col-md-12">
                ...
                >
                  ...
                  {(() => {
                    ...
                    const minYear = currentYear - 20;
                    const maxYear =
                      data.courseStatus === "Pursuing" ? currentYear + 5 : currentYear;
                    for (let year = maxYear; year >= minYear; year--) {
                      years.push(
                        <option key={year} value={year}>
                          {year}
                  ...
                  <div className="form-group col-lg-6 col-md-12">
                    ...
                    >
                      ...
                      {(() => {
                        ...
                        const minYear = currentYear - 16;
                        const maxYear =
                          data.courseStatus === "Pursuing" ? currentYear + 1 : currentYear;
                        for (let year = maxYear; year >= minYear; year--) {
                          years.push(
                            <option key={year} value={year}>
                              {year}
                ...
                <>
                  ...
                  <div className="form-group col-lg-6 col-md-12">
                    <input
                      ...
                      value={data.instituteName}
                      onChange={(e) =>
                        handleEducationDataChange(index, "instituteName", e.target.value)
                      }
                      placeholder="Institute Name"
                      maxLength={20}
                      pattern="[a-zA-Z0-9 ]*"
                  ...
                  <div className="form-group col-lg-6 col-md-12">
                    <input
                      ...
                      value={data.affiliatedTo}
                      onChange={(e) =>
                        handleEducationDataChange(index, "affiliatedTo", e.target.value)
                      }
                      placeholder="Affiliated to / recognized by"
                      maxLength={20}
                      pattern="[a-zA-Z0-9 ]*"
                  ...
                  <div className="form-group col-lg-6 col-md-12">
                    <Select
                      value={courseDurationOptions.find(
                        (option) => option.value === data.courseDuration
                      )}
                      onChange={(selected) =>
                        handleEducationDataChange(index, "courseDuration", selected.value)
                      }
                      options={courseDurationOptions}
                      placeholder="Course Duration"
                    />
          ...
          <div className="bed-section">
            <div className="row">
              ...
              {bEdCourseDuration && (
                <div className="form-group col-lg-6 col-md-12">
                  <Select
                    value={bEdCourseDurationOptions.find(
                      (option) => option.value === data.courseDuration
                    )}
                    onChange={(selected) =>
                      handleEducationDataChange(index, "courseDuration", selected.value)
            ...
            <div className="row">
              ...
              <div className="form-group col-lg-6 col-md-12">
                <input
                  ...
                  value={data.courseName}
                  onChange={(e) =>
                    handleEducationDataChange(index, "courseName", e.target.value)
                  }
                  placeholder="Course Name"
                  maxLength={20}
                  pattern="[a-zA-Z0-9 ]*"
              ...
              {certificateSpecialization && (
                <div className="form-group col-lg-6 col-md-12">
                  <input
                    ...
                    value={data.specialization}
                    onChange={(e) =>
                      handleEducationDataChange(index, "specialization", e.target.value)
                    }
                    placeholder="Specialization"
                    maxLength={20}
                    pattern="[a-zA-Z0-9 ]*"
            ...
            <div className="row">
              {isEasyMode ? (
                ...
                <>
                  ...
                  <div className="form-group col-lg-6 col-md-12">
                    <Select
                      value={dEdCourseDurationOptions.find(
                        (option) => option.value === data.courseDuration
                      )}
                      onChange={(selected) =>
                        handleEducationDataChange(index, "courseDuration", selected.value)
  ...
  // Submit handler: construct payload and send via Axios
  const handleSubmit = async (event) => {
    event.preventDefault();
          ...
          <div className="row">
            <div className="form-group col-lg-6 col-md-12">
              <select
                value=""
                onChange={(e) => {
                  const selected = educationTypes.find(
              ...
              >
                ...
                {educationTypes.map((type) => {
                  const alreadySelected = additionalEducation.some(
                    (edu) => edu.type === type.value
                  );
                  if (type.allowMultiple || !alreadySelected) {
                    return (
                      <option key={type.value} value={type.value}>
