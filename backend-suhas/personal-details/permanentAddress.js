...
const createAddress = async (body) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO permanent_address (firebase_uid, country_name, state_name, city_name, house_no_and_street, pincode)
      VALUES ?
    `;
...
const getAddress = async (event) => {
  ...
  try {
    const connection = await pool.getConnection();
    // If an "id" query parameter is provided, fetch that record; otherwise, return all.
