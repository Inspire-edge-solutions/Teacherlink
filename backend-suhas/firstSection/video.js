...
// POST /upload-image: Upload image to S3 and INSERT a new row with firebase_uid and the image key.
...
const generateVideoUploadUrlHandler = async (event) => {
  try {
    ...
    let result;
    if (id) {
      // Update the existing row with the video key and the firebase_uid.
      ...
      // No id provided: insert a new row with firebase_uid and the video key.
